...
// They prioritize hunting, mating, and eating, and run away from unknown opponents.
//
import java.awt.*;
import java.util.*;
...
public class Husky extends Critter {
   ...
   public Husky() { 
      ...
      knownFoes = new String[]{"S", "%", "V", "^", "<", ">", "0", "1", "2", "3", "4", "5", "6",
                               "7", "8"};
      numSections = 4;
      section = rand.nextInt(4) + 1;
      xNoise = this.getWidth() / (numSections * numSections * numSections);
      yNoise = this.getHeight() / (numSections * numSections * numSections);
   }
   ...
   public Direction getMove() { 
      ...
      for (int i=0; i < movesMap.length; i++) { 
         ...
         // priority 1: mating
         if (getNeighbor(movesMap[i]).equals("@") && !mated && !checkForFoes()) {
            return movesMap[i];
         }
      ...
      // get into your quadrant. Hard-coded for four qudrants
      if (section == 1) { 
         if (this.getX() >= (this.getWidth() / numSections * 2) + xNoise) { // go West to get back in
            return Direction.WEST;
         } else if (this.getX() >= this.getWidth() - xNoise) { // go East to get back in 
         ...
         if (this.getX() >= (this.getWidth() / numSections * 2) + xNoise) { // go West
            return Direction.WEST;
         } else if (this.getX() >= this.getWidth() - xNoise) { // go East to get back in 
            return Direction.EAST;
